...
public class generaion_map : MonoBehaviour
{
    ...
    public void OnMouseDown()
    {
        if (p == 5 && z == true)
        {
            ...
            drevo.GetComponent<Drevo_count>().add_drev += 17;
            drevo.GetComponent<Drevo_count>().cur_drev -= 1000;
            havka.GetComponent<havka_count>().cur_havka -= 100;
            kamen.GetComponent<kamen_count>().cur_kamen -= 1000;
        }
        if (p == 6 && z == true)
        {
        ...
        if (p == 7 && z == true)
        {
            ...
            GameObject.Find("hexagonAll_sheet_104").GetComponent<derev_stroit>().n = false;
            kamen.GetComponent<kamen_count>().add_kamen += 17;
            drevo.GetComponent<Drevo_count>().cur_drev -= 1000;
            havka.GetComponent<havka_count>().cur_havka -= 100;
            kamen.GetComponent<kamen_count>().cur_kamen -= 1000;
        }
        ...
        if (p == 9 && z == true)
        {
            ...
            GameObject.Find("hexagonAll_sheet_104").GetComponent<derev_stroit>().n = false;
            havka.GetComponent<havka_count>().add_havka += 17;
            drevo.GetComponent<Drevo_count>().cur_drev -= 1000;
            havka.GetComponent<havka_count>().cur_havka -= 100;
            kamen.GetComponent<kamen_count>().cur_kamen -= 1000;
        }
